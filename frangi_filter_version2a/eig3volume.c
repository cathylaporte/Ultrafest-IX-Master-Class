    ...
    if(mxGetClassID(prhs[0])==mxDOUBLE_CLASS) {
       /* Assign pointers to each input. */
        ...
        for(i=0; i<npixels; i++) {
            ...
            if(nlhs==6) {
	        /* orthogonal direction (largest eigenvector) */
	      /* modified by Cathy (second indices were 0) Jan 18, 2019 */
                Dvecx[i]=Davec[0][2];
                Dvecy[i]=Davec[1][2];
                Dvecz[i]=Davec[2][2];
            }
       ...
       /* Assign pointers to each input. */
        ...
        for(i=0; i<npixels; i++) {
            ...
            if(nlhs==6) {
                /* orthogonal direction (largest eigenvector) */
	      /* modified by Cathy (second indices were 0) Jan 18, 2019 */
                Dvecx_f[i]=(float)Davec[0][2]; 
                Dvecy_f[i]=(float)Davec[1][2]; 
                Dvecz_f[i]=(float)Davec[2][2];
            }
        }
